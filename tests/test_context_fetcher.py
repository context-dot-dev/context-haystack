...
class TestContextFetcher:
    ...
    def test_run_fetches_each_url(self, request_context: MagicMock) -> None:
    ...
    @patch("haystack_integrations.components.fetchers.context.context_fetcher.request_context")
    def test_run_runtime_params_replace_init_params(self, request_context: MagicMock) -> None:
        request_context.return_value = SCRAPE_RESPONSE
        fetcher = ContextFetcher(
    ...
    @pytest.mark.asyncio
    async def test_run_async_fetches_concurrently(self, request_context: AsyncMock) -> None:
